...
class MvcController{
	...
	public function SeleccionarProductosController(){
		...
		$respuesta = Datos::seleccionarProductosModel("productos",$_SESSION["id_tienda"]);
			...
			foreach($respuesta as $row => $item){
					...
					echo'<option>'.$item["id_producto"].' - '.$item["nombre"].'</option>';
	...
	public function registrarVentaController(){
		...
		if(isset($_POST["enviar"])){
			...
			/*$datosControllerVenta = array( "id_producto"=>$_POST["idProductoVenta"], 
									      "fechaVenta"=>$_POST["fechaVentaEditar"]);

	        $stockDisponible = Datos::seleccionarProductosVentaModel("productos", $_SESSION["id_tienda"], $datosControllerVenta);

			var_dump($_POST["idProductoVenta"]);
			var_dump($stockDisponible["stock"]);*/


				$datosControllerVenta = array( "id_producto"=>$_POST["idProductoVenta"], 
									      "fechaVenta"=>$_POST["fechaVentaEditar"]);

				$datosControllerProductosVendidos = array( "id_producto"=>$_POST["idProductoVenta"], 
									      					"cantidadVenta"=>$_POST["cantidadVenta"]);

				//print_r($datosControllerVenta);
				//print_r($datosControllerProductosVendidos);

				$precio_producto = Datos::seleccionarProductos2Model("productos",$_SESSION["id_tienda"],$datosControllerVenta);

				$datosControllerVenta = array( "id_producto"=>$_POST["idProductoVenta"], 
									      		"fechaVenta"=>$_POST["fechaVentaEditar"],
									  			"total"=>$precio_producto["preciounitario"] * $_POST["cantidadVenta"]);
				...
				//Se le dice al modelo models/crud.php (Datos::registroUsuarioModel),que en la clase "Datos", la funcion "registroUsuarioModel" reciba en sus 2 parametros los valores "$datosController" y el nombre de la tabla a conectarnos la cual es "usuarios":
				$insert_venta = Datos::registroVentaModel($datosControllerVenta, "ventas",$_SESSION["id_tienda"]);

				//se imprime la respuesta en la vista 
				if($insert_venta == "success"){

					$selec_ultima_venta = Datos::seleccionarUltimaVentaModel("ventas",$_SESSION["id_tienda"]);

					$datosControllerProductosVendidos = array( "id_producto"=>$_POST["idProductoVenta"], 
						      									"cantidadVenta"=>$_POST["cantidadVenta"],
						      									"total"=>$precio_producto["preciounitario"] * $_POST["cantidadVenta"],
						      									"id_venta"=>$selec_ultima_venta["id_venta"]);
					...
					if ($insert_prod_vendidos == "success") {
						...
						$selectIDproducto = Datos::seleccionarIDProductoModel("productos_vendidos",$_SESSION["id_tienda"]);

						//$select_ultimoprod_vendido = Datos::seleccionarUltimoProdVendidoModel("productos_vendidos",$_SESSION["id_tienda"]);


						$datosController = array( "id_producto"=>$selectIDproducto["id_producto"],
												  "id_usuario"=>$_SESSION["id_usuario"],
												  "fechaMov"=>$_POST["fechaVentaEditar"],
							                  	  "nota"=>"El usuario ".$_SESSION["nombre"]." vendió ".$_POST["cantidadVenta"]." unidades",
						...
						if ($respuesta2 == "success") {
							...
							$productos_vendidos = Datos::restarStockProductosModel($datosControllerProductosVendidos, "productos");

							if ($productos_vendidos == "success") {
	...
	public function vistaProductosVendController(){
		...
		$respuesta = Datos::vistaProductosVendModel("productos_vendidos",$_SESSION["id_tienda"],$datosController);
	...
	public function vistaVentasController(){
        ...
        echo '<script type="text/javascript">
		    ...
		    function borrar(){
		      ...
		      .then((value) => {
		          if (value==password) {
		            window.location.href = "index.php?action=ventas&idBorrar='.$item["id_venta"].'";
		          }else{
		            swal("Contraseña Incorrecta", "Intente de nuevo", "error");
		          }     
		      });
		...
		if(isset($_POST["actualizar"])){
			...
			$datosController = array( "id_producto"=>$_POST["idEditar"],
				                      ...
				                      "categoria"=>$_POST["categoriaEditar"],
				                  	  ...
				                  	  "nota"=>/*$_POST["notaEditar"],*/"El usuario ".$_SESSION["nombre"]." agrego ".$_POST["stockAgregarEditar"]." unidades",
	...
	public function actualizarretiroStockProductoController(){
		...
		if(isset($_POST["actualizar"])){
			...
			$datosController = array( "id_producto"=>$_POST["idEditar"],
				                      ...
				                      "categoria"=>$_POST["categoriaEditar"],
				                  	  "cantidad"=>$_POST["stockAgregarEditar"],
				                  	  "nota"=>/*$_POST["notaEditar"],*/"El usuario ".$_SESSION["nombre"]." quito ".$_POST["stockAgregarEditar"]." unidades",
				                  	  "referencia"=>$_POST["referenciaEditar"]);
...
/*	public function borrarVentasController(){
		...
		if(isset($_GET["idBorrar"])){
			...
			$respuesta = Datos::borrarVentaModel($datosController, "ventas",$_SESSION["id_tienda"]);
			//$respuesta2 = Datos::borrarCategoriaModel($datosController, "categorias");
